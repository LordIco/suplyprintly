...
function showLogin(message=''){document.body.classList.add('auth-locked');const login=$('#loginScreen'),app=$('#adminApp');login.hidden=false;login.style.display='grid';app.hidden=true;app.style.display='none';$('#loginMessage').textContent=message;setTimeout(()=>$('#loginEmail')?.focus(),0)}
function showApp(){document.body.classList.remove('auth-locked');const login=$('#loginScreen'),app=$('#adminApp');login.hidden=true;login.style.display='none';app.hidden=false;app.style.display='block';$('#configMissing').hidden=true;$('#configMissing').style.display='none';$('#userEmail').textContent=currentUser?.email||'Administrador';window.scrollTo({top:0,behavior:'auto'})}
async function bootstrap(){
  if(!configured()){
    $('#configMissing').hidden=false;$('#configMissing').style.display='grid';showLogin('O site ainda não foi conectado ao Supabase. Abra assets/js/config.js e faça a configuração indicada no README.');$('#loginForm').querySelector('button').disabled=true;return;
  }
  $('#configMissing').hidden=true;$('#configMissing').style.display='none';$('#loginForm').querySelector('button').disabled=false;
  try{
...
$('#loginForm').addEventListener('submit',async e=>{e.preventDefault();if(!configured())return;const email=$('#loginEmail').value.trim(),password=$('#loginPassword').value;$('#loginMessage').textContent='Validando acesso…';try{setBusy(true);const {data,error}=await client().auth.signInWithPassword({email,password});if(error)throw error;currentUser=data.user;if(!(await isAdmin())){await client().auth.signOut();throw new Error('Usuário sem permissão administrativa.')}showApp();await loadProducts();$('#loginMessage').textContent='';toast('Acesso liberado.','ok')}catch(err){console.error(err);$('#loginMessage').textContent=err.message==='Invalid login credentials'?'E-mail ou senha inválidos.':err.message||'Falha no login.'}finally{setBusy(false)}});
